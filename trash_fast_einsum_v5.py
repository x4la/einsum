...
def bmm_wrapper(A, B, shape_a, shape_b, stride_a, stride_b):
    b, m, k = shape_a
    b_, k_, n = shape_b
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))
    dll_path = os.path.join(current_dir, 'cmake-build-debug', 'libbmm_v5.dll')
    lib = ctypes.CDLL(dll_path)
    # set up C++ function
    lib.my_bmm.argtypes = [
        ctypes.c_void_p,  # Pointer to A
        ctypes.c_void_p,  # Pointer to B
        ctypes.c_void_p,  # Pointer to C
        ctypes.POINTER(ctypes.c_int),  # Strides of A
        ctypes.POINTER(ctypes.c_int),  # Strides of B
        ctypes.c_int,     # b
        ctypes.c_int,     # m
        ctypes.c_int,     # n
        ctypes.c_int      # k
    ...
    lib.my_bmm(
        ...
        B.ctypes.data_as(ctypes.c_void_p),
        C.ctypes.data_as(ctypes.c_void_p),
        stride_a.ctypes.data_as(ctypes.POINTER(ctypes.c_int)),
        stride_b.ctypes.data_as(ctypes.POINTER(ctypes.c_int)),
        ctypes.c_int(b),
        ctypes.c_int(m),
        ctypes.c_int(n),
        ctypes.c_int(k),
...
def custom_reshape(idx, act_stride):
    stride = (0, 0, 0)
...
def fast_einsum(subscripts, a, b=None):
    ...
    if b is None:
        ...
        a = single_einsum(str_a, a)
        if np.isscalar(a):
            a = np.array([a])
        act_stride_a = tuple(s // a.itemsize for s in a.strides)
        b = single_einsum(str_b, b)
        if np.isscalar(b):
            b = np.array([b])
        act_stride_b = tuple(s // b.itemsize for s in b.strides)
        # print(b.shape, act_stride_b)
        stride_a = custom_reshape(idx_a, act_stride_a)
        stride_b = custom_reshape(idx_b, act_stride_b)
        # stride_a = np.array([0, 1, 24], dtype=np.int32)
        # stride_b = np.array([0, 1, 38], dtype=np.int32)
        stride_a = np.array([0, 1, 84**3], dtype=np.int32)
        stride_b = np.array([0, 1, 94], dtype=np.int32)
        c = bmm_wrapper(a, b, shape_a, shape_b, stride_a, stride_b).reshape(shape_res)
...
def parse(s, shape_a, shape_b):
    ...
    str_a = "".join(dict.fromkeys(str_a_in))  # remove duplicates
    str_b = "".join(dict.fromkeys(str_b_in))  # remove duplicates
    ...
    idx_a = (batch_idxs, left_idxs, contract_idxs)
    idx_b = (batch_idxs, contract_idxs, right_idxs)

    return str_a, shape_a, idx_a, str_b, shape_b, idx_b, str_res, shape_res
...
def remove_diags(s, strides, shape):
    """ contains the main logic to remove diagonals
        ...
        params: s           - str containing indices of a tensor.       Ex: "ijkijk"
                strides     - strides of tensor.                        Ex: (256, 128, 64, 32, 16, 8) (double = 8 byte)
