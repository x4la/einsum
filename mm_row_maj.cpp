...
void matmul(float* B, float* A, float* C, int n, int m, int k) {
...
extern "C" {
    void my_mm(
    void* A,       // Pointer to A with dims (m, k)
    void* B,       // Pointer to B with dims (k, n)
    void* C,       // Pointer to C with dims (m, n)
    int m,
    int n,
    int k
) {
      matmul(static_cast<float*>(A), static_cast<float*>(B), static_cast<float*>(C), m, n, k);
    }
