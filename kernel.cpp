#include "kernel.h"

#include <cstdio>

void kernel_16x6_float(float* blockA_packed,
                       float* blockB_packed,
                       float* C,
                       int mr,
                       int nr,
                       int kc,
                       int m) {
    __m256 C00 = _mm256_setzero_ps();
    __m256 C10 = _mm256_setzero_ps();
    __m256 C01 = _mm256_setzero_ps();
    __m256 C11 = _mm256_setzero_ps();
    __m256 C02 = _mm256_setzero_ps();
    __m256 C12 = _mm256_setzero_ps();
    __m256 C03 = _mm256_setzero_ps();
    __m256 C13 = _mm256_setzero_ps();
    __m256 C04 = _mm256_setzero_ps();
    __m256 C14 = _mm256_setzero_ps();
    __m256 C05 = _mm256_setzero_ps();
    __m256 C15 = _mm256_setzero_ps();

    __m256 b_packFloat8;
    __m256 a0_packFloat8;
    __m256 a1_packFloat8;
    __m256i packed_mask0;
    __m256i packed_mask1;
    if (mr != 16) {
        packed_mask0 = _mm256_cvtepi8_epi32(_mm_loadu_si64(&mask[16 - mr]));
        packed_mask1 = _mm256_cvtepi8_epi32(_mm_loadu_si64(&mask[16 - mr + 8]));
        switch (nr) {
        case 1 :
            C00 = _mm256_maskload_ps(C, packed_mask0);
            C10 = _mm256_maskload_ps(&C[8], packed_mask1);
            break;
        case 2 :
            C00 = _mm256_maskload_ps(C, packed_mask0);
            C10 = _mm256_maskload_ps(&C[8], packed_mask1);
            C01 = _mm256_maskload_ps(&C[m], packed_mask0);
            C11 = _mm256_maskload_ps(&C[m + 8], packed_mask1);
            break;
        case 3 :
            C00 = _mm256_maskload_ps(C, packed_mask0);
            C10 = _mm256_maskload_ps(&C[8], packed_mask1);
            C01 = _mm256_maskload_ps(&C[m], packed_mask0);
            C11 = _mm256_maskload_ps(&C[m + 8], packed_mask1);
            C02 = _mm256_maskload_ps(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_ps(&C[2 * m + 8], packed_mask1);
            break;
        case 4 :
            C00 = _mm256_maskload_ps(C, packed_mask0);
            C10 = _mm256_maskload_ps(&C[8], packed_mask1);
            C01 = _mm256_maskload_ps(&C[m], packed_mask0);
            C11 = _mm256_maskload_ps(&C[m + 8], packed_mask1);
            C02 = _mm256_maskload_ps(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_ps(&C[2 * m + 8], packed_mask1);
            C03 = _mm256_maskload_ps(&C[3 * m], packed_mask0);
            C13 = _mm256_maskload_ps(&C[3 * m + 8], packed_mask1);
            break;
        case 5 :
            C00 = _mm256_maskload_ps(C, packed_mask0);
            C10 = _mm256_maskload_ps(&C[8], packed_mask1);
            C01 = _mm256_maskload_ps(&C[m], packed_mask0);
            C11 = _mm256_maskload_ps(&C[m + 8], packed_mask1);
            C02 = _mm256_maskload_ps(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_ps(&C[2 * m + 8], packed_mask1);
            C03 = _mm256_maskload_ps(&C[3 * m], packed_mask0);
            C13 = _mm256_maskload_ps(&C[3 * m + 8], packed_mask1);
            C04 = _mm256_maskload_ps(&C[4 * m], packed_mask0);
            C14 = _mm256_maskload_ps(&C[4 * m + 8], packed_mask1);
            break;
        case 6 :
            C00 = _mm256_maskload_ps(C, packed_mask0);
            C10 = _mm256_maskload_ps(&C[8], packed_mask1);
            C01 = _mm256_maskload_ps(&C[m], packed_mask0);
            C11 = _mm256_maskload_ps(&C[m + 8], packed_mask1);
            C02 = _mm256_maskload_ps(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_ps(&C[2 * m + 8], packed_mask1);
            C03 = _mm256_maskload_ps(&C[3 * m], packed_mask0);
            C13 = _mm256_maskload_ps(&C[3 * m + 8], packed_mask1);
            C04 = _mm256_maskload_ps(&C[4 * m], packed_mask0);
            C14 = _mm256_maskload_ps(&C[4 * m + 8], packed_mask1);
            C05 = _mm256_maskload_ps(&C[5 * m], packed_mask0);
            C15 = _mm256_maskload_ps(&C[5 * m + 8], packed_mask1);
            break;
        }
    } else {
        switch (nr) {
        case 1 :
            C00 = _mm256_loadu_ps(C);
            C10 = _mm256_loadu_ps(&C[8]);
            break;
        case 2 :
            C00 = _mm256_loadu_ps(C);
            C10 = _mm256_loadu_ps(&C[8]);
            C01 = _mm256_loadu_ps(&C[m]);
            C11 = _mm256_loadu_ps(&C[m + 8]);
            break;
        case 3 :
            C00 = _mm256_loadu_ps(C);
            C10 = _mm256_loadu_ps(&C[8]);
            C01 = _mm256_loadu_ps(&C[m]);
            C11 = _mm256_loadu_ps(&C[m + 8]);
            C02 = _mm256_loadu_ps(&C[2 * m]);
            C12 = _mm256_loadu_ps(&C[2 * m + 8]);
            break;
        case 4 :
            C00 = _mm256_loadu_ps(C);
            C10 = _mm256_loadu_ps(&C[8]);
            C01 = _mm256_loadu_ps(&C[m]);
            C11 = _mm256_loadu_ps(&C[m + 8]);
            C02 = _mm256_loadu_ps(&C[2 * m]);
            C12 = _mm256_loadu_ps(&C[2 * m + 8]);
            C03 = _mm256_loadu_ps(&C[3 * m]);
            C13 = _mm256_loadu_ps(&C[3 * m + 8]);
            break;
        case 5 :
            C00 = _mm256_loadu_ps(C);
            C10 = _mm256_loadu_ps(&C[8]);
            C01 = _mm256_loadu_ps(&C[m]);
            C11 = _mm256_loadu_ps(&C[m + 8]);
            C02 = _mm256_loadu_ps(&C[2 * m]);
            C12 = _mm256_loadu_ps(&C[2 * m + 8]);
            C03 = _mm256_loadu_ps(&C[3 * m]);
            C13 = _mm256_loadu_ps(&C[3 * m + 8]);
            C04 = _mm256_loadu_ps(&C[4 * m]);
            C14 = _mm256_loadu_ps(&C[4 * m + 8]);
            break;
        case 6 :
            C00 = _mm256_loadu_ps(C);
            C10 = _mm256_loadu_ps(&C[8]);
            C01 = _mm256_loadu_ps(&C[m]);
            C11 = _mm256_loadu_ps(&C[m + 8]);
            C02 = _mm256_loadu_ps(&C[2 * m]);
            C12 = _mm256_loadu_ps(&C[2 * m + 8]);
            C03 = _mm256_loadu_ps(&C[3 * m]);
            C13 = _mm256_loadu_ps(&C[3 * m + 8]);
            C04 = _mm256_loadu_ps(&C[4 * m]);
            C14 = _mm256_loadu_ps(&C[4 * m + 8]);
            C05 = _mm256_loadu_ps(&C[5 * m]);
            C15 = _mm256_loadu_ps(&C[5 * m + 8]);
            break;
        }
    }
    for (int p = 0; p < kc; p++) {
        a0_packFloat8 = _mm256_loadu_ps(blockA_packed);
        a1_packFloat8 = _mm256_loadu_ps(blockA_packed + 8);

        b_packFloat8 = _mm256_broadcast_ss(blockB_packed);
        C00 = _mm256_fmadd_ps(a0_packFloat8, b_packFloat8, C00);
        C10 = _mm256_fmadd_ps(a1_packFloat8, b_packFloat8, C10);

        b_packFloat8 = _mm256_broadcast_ss(blockB_packed + 1);
        C01 = _mm256_fmadd_ps(a0_packFloat8, b_packFloat8, C01);
        C11 = _mm256_fmadd_ps(a1_packFloat8, b_packFloat8, C11);

        b_packFloat8 = _mm256_broadcast_ss(blockB_packed + 2);
        C02 = _mm256_fmadd_ps(a0_packFloat8, b_packFloat8, C02);
        C12 = _mm256_fmadd_ps(a1_packFloat8, b_packFloat8, C12);

        b_packFloat8 = _mm256_broadcast_ss(blockB_packed + 3);
        C03 = _mm256_fmadd_ps(a0_packFloat8, b_packFloat8, C03);
        C13 = _mm256_fmadd_ps(a1_packFloat8, b_packFloat8, C13);

        b_packFloat8 = _mm256_broadcast_ss(blockB_packed + 4);
        C04 = _mm256_fmadd_ps(a0_packFloat8, b_packFloat8, C04);
        C14 = _mm256_fmadd_ps(a1_packFloat8, b_packFloat8, C14);

        b_packFloat8 = _mm256_broadcast_ss(blockB_packed + 5);
        C05 = _mm256_fmadd_ps(a0_packFloat8, b_packFloat8, C05);
        C15 = _mm256_fmadd_ps(a1_packFloat8, b_packFloat8, C15);

        blockA_packed += 16;
        blockB_packed += 6;
    }
    if (mr != 16) {
        switch (nr) {
        case 1 :
            _mm256_maskstore_ps(C, packed_mask0, C00);
            _mm256_maskstore_ps(&C[8], packed_mask1, C10);
            break;
        case 2 :
            _mm256_maskstore_ps(C, packed_mask0, C00);
            _mm256_maskstore_ps(&C[8], packed_mask1, C10);
            _mm256_maskstore_ps(&C[m], packed_mask0, C01);
            _mm256_maskstore_ps(&C[m + 8], packed_mask1, C11);
            break;
        case 3 :
            _mm256_maskstore_ps(C, packed_mask0, C00);
            _mm256_maskstore_ps(&C[8], packed_mask1, C10);
            _mm256_maskstore_ps(&C[m], packed_mask0, C01);
            _mm256_maskstore_ps(&C[m + 8], packed_mask1, C11);
            _mm256_maskstore_ps(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_ps(&C[2 * m + 8], packed_mask1, C12);
            break;
        case 4 :
            _mm256_maskstore_ps(C, packed_mask0, C00);
            _mm256_maskstore_ps(&C[8], packed_mask1, C10);
            _mm256_maskstore_ps(&C[m], packed_mask0, C01);
            _mm256_maskstore_ps(&C[m + 8], packed_mask1, C11);
            _mm256_maskstore_ps(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_ps(&C[2 * m + 8], packed_mask1, C12);
            _mm256_maskstore_ps(&C[3 * m], packed_mask0, C03);
            _mm256_maskstore_ps(&C[3 * m + 8], packed_mask1, C13);
            break;
        case 5 :
            _mm256_maskstore_ps(C, packed_mask0, C00);
            _mm256_maskstore_ps(&C[8], packed_mask1, C10);
            _mm256_maskstore_ps(&C[m], packed_mask0, C01);
            _mm256_maskstore_ps(&C[m + 8], packed_mask1, C11);
            _mm256_maskstore_ps(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_ps(&C[2 * m + 8], packed_mask1, C12);
            _mm256_maskstore_ps(&C[3 * m], packed_mask0, C03);
            _mm256_maskstore_ps(&C[3 * m + 8], packed_mask1, C13);
            _mm256_maskstore_ps(&C[4 * m], packed_mask0, C04);
            _mm256_maskstore_ps(&C[4 * m + 8], packed_mask1, C14);
            break;
        case 6 :
            _mm256_maskstore_ps(C, packed_mask0, C00);
            _mm256_maskstore_ps(&C[8], packed_mask1, C10);
            _mm256_maskstore_ps(&C[m], packed_mask0, C01);
            _mm256_maskstore_ps(&C[m + 8], packed_mask1, C11);
            _mm256_maskstore_ps(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_ps(&C[2 * m + 8], packed_mask1, C12);
            _mm256_maskstore_ps(&C[3 * m], packed_mask0, C03);
            _mm256_maskstore_ps(&C[3 * m + 8], packed_mask1, C13);
            _mm256_maskstore_ps(&C[4 * m], packed_mask0, C04);
            _mm256_maskstore_ps(&C[4 * m + 8], packed_mask1, C14);
            _mm256_maskstore_ps(&C[5 * m], packed_mask0, C05);
            _mm256_maskstore_ps(&C[5 * m + 8], packed_mask1, C15);
            break;
        }
    } else {
        switch (nr) {
        case 1 :
            _mm256_storeu_ps(C, C00);
            _mm256_storeu_ps(&C[8], C10);
            break;
        case 2 :
            _mm256_storeu_ps(C, C00);
            _mm256_storeu_ps(&C[8], C10);
            _mm256_storeu_ps(&C[m], C01);
            _mm256_storeu_ps(&C[m + 8], C11);
            break;
        case 3 :
            _mm256_storeu_ps(C, C00);
            _mm256_storeu_ps(&C[8], C10);
            _mm256_storeu_ps(&C[m], C01);
            _mm256_storeu_ps(&C[m + 8], C11);
            _mm256_storeu_ps(&C[2 * m], C02);
            _mm256_storeu_ps(&C[2 * m + 8], C12);
            break;
        case 4 :
            _mm256_storeu_ps(C, C00);
            _mm256_storeu_ps(&C[8], C10);
            _mm256_storeu_ps(&C[m], C01);
            _mm256_storeu_ps(&C[m + 8], C11);
            _mm256_storeu_ps(&C[2 * m], C02);
            _mm256_storeu_ps(&C[2 * m + 8], C12);
            _mm256_storeu_ps(&C[3 * m], C03);
            _mm256_storeu_ps(&C[3 * m + 8], C13);
            break;
        case 5 :
            _mm256_storeu_ps(C, C00);
            _mm256_storeu_ps(&C[8], C10);
            _mm256_storeu_ps(&C[m], C01);
            _mm256_storeu_ps(&C[m + 8], C11);
            _mm256_storeu_ps(&C[2 * m], C02);
            _mm256_storeu_ps(&C[2 * m + 8], C12);
            _mm256_storeu_ps(&C[3 * m], C03);
            _mm256_storeu_ps(&C[3 * m + 8], C13);
            _mm256_storeu_ps(&C[4 * m], C04);
            _mm256_storeu_ps(&C[4 * m + 8], C14);
            break;
        case 6 :
            _mm256_storeu_ps(C, C00);
            _mm256_storeu_ps(&C[8], C10);
            _mm256_storeu_ps(&C[m], C01);
            _mm256_storeu_ps(&C[m + 8], C11);
            _mm256_storeu_ps(&C[2 * m], C02);
            _mm256_storeu_ps(&C[2 * m + 8], C12);
            _mm256_storeu_ps(&C[3 * m], C03);
            _mm256_storeu_ps(&C[3 * m + 8], C13);
            _mm256_storeu_ps(&C[4 * m], C04);
            _mm256_storeu_ps(&C[4 * m + 8], C14);
            _mm256_storeu_ps(&C[5 * m], C05);
            _mm256_storeu_ps(&C[5 * m + 8], C15);
            break;
        }
    }
}

void kernel_8x6_double(double* blockA_packed,
                 double* blockB_packed,
                 double* C,
                 int mr,
                 int nr,
                 int kc,
                 int m) {
    __m256d C00 = _mm256_setzero_pd();
    __m256d C10 = _mm256_setzero_pd();
    __m256d C01 = _mm256_setzero_pd();
    __m256d C11 = _mm256_setzero_pd();
    __m256d C02 = _mm256_setzero_pd();
    __m256d C12 = _mm256_setzero_pd();
    __m256d C03 = _mm256_setzero_pd();
    __m256d C13 = _mm256_setzero_pd();
    __m256d C04 = _mm256_setzero_pd();
    __m256d C14 = _mm256_setzero_pd();
    __m256d C05 = _mm256_setzero_pd();
    __m256d C15 = _mm256_setzero_pd();

    __m256d b_packDouble4;
    __m256d a0_packDouble4;
    __m256d a1_packDouble4;
    __m256i packed_mask0;
    __m256i packed_mask1;
    if (mr != 8) {
        packed_mask0 = _mm256_cvtepi8_epi64(_mm_loadu_si64(&mask_double[8 - mr]));
        packed_mask1 = _mm256_cvtepi8_epi64(_mm_loadu_si64(&mask_double[8 - mr + 4]));
        switch (nr) {
        case 1 :
            C00 = _mm256_maskload_pd(C, packed_mask0);
            C10 = _mm256_maskload_pd(&C[4], packed_mask1);
            break;
        case 2 :
            C00 = _mm256_maskload_pd(C, packed_mask0);
            C10 = _mm256_maskload_pd(&C[4], packed_mask1);
            C01 = _mm256_maskload_pd(&C[m], packed_mask0);
            C11 = _mm256_maskload_pd(&C[m + 4], packed_mask1);
            break;
        case 3 :
            C00 = _mm256_maskload_pd(C, packed_mask0);
            C10 = _mm256_maskload_pd(&C[4], packed_mask1);
            C01 = _mm256_maskload_pd(&C[m], packed_mask0);
            C11 = _mm256_maskload_pd(&C[m + 4], packed_mask1);
            C02 = _mm256_maskload_pd(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_pd(&C[2 * m + 4], packed_mask1);
            break;
        case 4 :
            C00 = _mm256_maskload_pd(C, packed_mask0);
            C10 = _mm256_maskload_pd(&C[4], packed_mask1);
            C01 = _mm256_maskload_pd(&C[m], packed_mask0);
            C11 = _mm256_maskload_pd(&C[m + 4], packed_mask1);
            C02 = _mm256_maskload_pd(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_pd(&C[2 * m + 4], packed_mask1);
            C03 = _mm256_maskload_pd(&C[3 * m], packed_mask0);
            C13 = _mm256_maskload_pd(&C[3 * m + 4], packed_mask1);
            break;
        case 5 :
            C00 = _mm256_maskload_pd(C, packed_mask0);
            C10 = _mm256_maskload_pd(&C[4], packed_mask1);
            C01 = _mm256_maskload_pd(&C[m], packed_mask0);
            C11 = _mm256_maskload_pd(&C[m + 4], packed_mask1);
            C02 = _mm256_maskload_pd(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_pd(&C[2 * m + 4], packed_mask1);
            C03 = _mm256_maskload_pd(&C[3 * m], packed_mask0);
            C13 = _mm256_maskload_pd(&C[3 * m + 4], packed_mask1);
            C04 = _mm256_maskload_pd(&C[4 * m], packed_mask0);
            C14 = _mm256_maskload_pd(&C[4 * m + 4], packed_mask1);
            break;
        case 6 :
            C00 = _mm256_maskload_pd(C, packed_mask0);
            C10 = _mm256_maskload_pd(&C[4], packed_mask1);
            C01 = _mm256_maskload_pd(&C[m], packed_mask0);
            C11 = _mm256_maskload_pd(&C[m + 4], packed_mask1);
            C02 = _mm256_maskload_pd(&C[2 * m], packed_mask0);
            C12 = _mm256_maskload_pd(&C[2 * m + 4], packed_mask1);
            C03 = _mm256_maskload_pd(&C[3 * m], packed_mask0);
            C13 = _mm256_maskload_pd(&C[3 * m + 4], packed_mask1);
            C04 = _mm256_maskload_pd(&C[4 * m], packed_mask0);
            C14 = _mm256_maskload_pd(&C[4 * m + 4], packed_mask1);
            C05 = _mm256_maskload_pd(&C[5 * m], packed_mask0);
            C15 = _mm256_maskload_pd(&C[5 * m + 4], packed_mask1);
            break;
        }
    } else {
        switch (nr) {
        case 1 :
            C00 = _mm256_loadu_pd(C);
            C10 = _mm256_loadu_pd(&C[4]);
            break;
        case 2 :
            C00 = _mm256_loadu_pd(C);
            C10 = _mm256_loadu_pd(&C[4]);
            C01 = _mm256_loadu_pd(&C[m]);
            C11 = _mm256_loadu_pd(&C[m + 4]);
            break;
        case 3 :
            C00 = _mm256_loadu_pd(C);
            C10 = _mm256_loadu_pd(&C[4]);
            C01 = _mm256_loadu_pd(&C[m]);
            C11 = _mm256_loadu_pd(&C[m + 4]);
            C02 = _mm256_loadu_pd(&C[2 * m]);
            C12 = _mm256_loadu_pd(&C[2 * m + 4]);
            break;
        case 4 :
            C00 = _mm256_loadu_pd(C);
            C10 = _mm256_loadu_pd(&C[4]);
            C01 = _mm256_loadu_pd(&C[m]);
            C11 = _mm256_loadu_pd(&C[m + 4]);
            C02 = _mm256_loadu_pd(&C[2 * m]);
            C12 = _mm256_loadu_pd(&C[2 * m + 4]);
            C03 = _mm256_loadu_pd(&C[3 * m]);
            C13 = _mm256_loadu_pd(&C[3 * m + 4]);
            break;
        case 5 :
            C00 = _mm256_loadu_pd(C);
            C10 = _mm256_loadu_pd(&C[4]);
            C01 = _mm256_loadu_pd(&C[m]);
            C11 = _mm256_loadu_pd(&C[m + 4]);
            C02 = _mm256_loadu_pd(&C[2 * m]);
            C12 = _mm256_loadu_pd(&C[2 * m + 4]);
            C03 = _mm256_loadu_pd(&C[3 * m]);
            C13 = _mm256_loadu_pd(&C[3 * m + 4]);
            C04 = _mm256_loadu_pd(&C[4 * m]);
            C14 = _mm256_loadu_pd(&C[4 * m + 4]);
            break;
        case 6 :
            C00 = _mm256_loadu_pd(C);
            C10 = _mm256_loadu_pd(&C[4]);
            C01 = _mm256_loadu_pd(&C[m]);
            C11 = _mm256_loadu_pd(&C[m + 4]);
            C02 = _mm256_loadu_pd(&C[2 * m]);
            C12 = _mm256_loadu_pd(&C[2 * m + 4]);
            C03 = _mm256_loadu_pd(&C[3 * m]);
            C13 = _mm256_loadu_pd(&C[3 * m + 4]);
            C04 = _mm256_loadu_pd(&C[4 * m]);
            C14 = _mm256_loadu_pd(&C[4 * m + 4]);
            C05 = _mm256_loadu_pd(&C[5 * m]);
            C15 = _mm256_loadu_pd(&C[5 * m + 4]);
            break;
        }
    }
    for (int p = 0; p < kc; p++) {
        a0_packDouble4= _mm256_loadu_pd(blockA_packed);
        a1_packDouble4 = _mm256_loadu_pd(blockA_packed + 4);

        b_packDouble4 = _mm256_broadcast_sd(blockB_packed);
        C00 = _mm256_fmadd_pd(a0_packDouble4, b_packDouble4, C00);
        C10 = _mm256_fmadd_pd(a1_packDouble4, b_packDouble4, C10);

        b_packDouble4 = _mm256_broadcast_sd(blockB_packed + 1);
        C01 = _mm256_fmadd_pd(a0_packDouble4, b_packDouble4, C01);
        C11 = _mm256_fmadd_pd(a1_packDouble4, b_packDouble4, C11);

        b_packDouble4 = _mm256_broadcast_sd(blockB_packed + 2);
        C02 = _mm256_fmadd_pd(a0_packDouble4, b_packDouble4, C02);
        C12 = _mm256_fmadd_pd(a1_packDouble4, b_packDouble4, C12);

        b_packDouble4 = _mm256_broadcast_sd(blockB_packed + 3);
        C03 = _mm256_fmadd_pd(a0_packDouble4, b_packDouble4, C03);
        C13 = _mm256_fmadd_pd(a1_packDouble4, b_packDouble4, C13);

        b_packDouble4 = _mm256_broadcast_sd(blockB_packed + 4);
        C04 = _mm256_fmadd_pd(a0_packDouble4, b_packDouble4, C04);
        C14 = _mm256_fmadd_pd(a1_packDouble4, b_packDouble4, C14);

        b_packDouble4 = _mm256_broadcast_sd(blockB_packed + 5);
        C05 = _mm256_fmadd_pd(a0_packDouble4, b_packDouble4, C05);
        C15 = _mm256_fmadd_pd(a1_packDouble4, b_packDouble4, C15);

        blockA_packed += 8;
        blockB_packed += 6;
    }
    if (mr != 8) {
        switch (nr) {
        case 1 :
            _mm256_maskstore_pd(C, packed_mask0, C00);
            _mm256_maskstore_pd(&C[4], packed_mask1, C10);
            break;
        case 2 :
            _mm256_maskstore_pd(C, packed_mask0, C00);
            _mm256_maskstore_pd(&C[4], packed_mask1, C10);
            _mm256_maskstore_pd(&C[m], packed_mask0, C01);
            _mm256_maskstore_pd(&C[m + 4], packed_mask1, C11);
            break;
        case 3 :
            _mm256_maskstore_pd(C, packed_mask0, C00);
            _mm256_maskstore_pd(&C[4], packed_mask1, C10);
            _mm256_maskstore_pd(&C[m], packed_mask0, C01);
            _mm256_maskstore_pd(&C[m + 4], packed_mask1, C11);
            _mm256_maskstore_pd(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_pd(&C[2 * m + 4], packed_mask1, C12);
            break;
        case 4 :
            _mm256_maskstore_pd(C, packed_mask0, C00);
            _mm256_maskstore_pd(&C[4], packed_mask1, C10);
            _mm256_maskstore_pd(&C[m], packed_mask0, C01);
            _mm256_maskstore_pd(&C[m + 4], packed_mask1, C11);
            _mm256_maskstore_pd(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_pd(&C[2 * m + 4], packed_mask1, C12);
            _mm256_maskstore_pd(&C[3 * m], packed_mask0, C03);
            _mm256_maskstore_pd(&C[3 * m + 4], packed_mask1, C13);
            break;
        case 5 :
            _mm256_maskstore_pd(C, packed_mask0, C00);
            _mm256_maskstore_pd(&C[4], packed_mask1, C10);
            _mm256_maskstore_pd(&C[m], packed_mask0, C01);
            _mm256_maskstore_pd(&C[m + 4], packed_mask1, C11);
            _mm256_maskstore_pd(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_pd(&C[2 * m + 4], packed_mask1, C12);
            _mm256_maskstore_pd(&C[3 * m], packed_mask0, C03);
            _mm256_maskstore_pd(&C[3 * m + 4], packed_mask1, C13);
            _mm256_maskstore_pd(&C[4 * m], packed_mask0, C04);
            _mm256_maskstore_pd(&C[4 * m + 4], packed_mask1, C14);
            break;
        case 6 :
            _mm256_maskstore_pd(C, packed_mask0, C00);
            _mm256_maskstore_pd(&C[4], packed_mask1, C10);
            _mm256_maskstore_pd(&C[m], packed_mask0, C01);
            _mm256_maskstore_pd(&C[m + 4], packed_mask1, C11);
            _mm256_maskstore_pd(&C[2 * m], packed_mask0, C02);
            _mm256_maskstore_pd(&C[2 * m + 4], packed_mask1, C12);
            _mm256_maskstore_pd(&C[3 * m], packed_mask0, C03);
            _mm256_maskstore_pd(&C[3 * m + 4], packed_mask1, C13);
            _mm256_maskstore_pd(&C[4 * m], packed_mask0, C04);
            _mm256_maskstore_pd(&C[4 * m + 4], packed_mask1, C14);
            _mm256_maskstore_pd(&C[5 * m], packed_mask0, C05);
            _mm256_maskstore_pd(&C[5 * m + 4], packed_mask1, C15);
            break;
        }
    } else {
        switch (nr) {
        case 1 :
            _mm256_storeu_pd(C, C00);
            _mm256_storeu_pd(&C[4], C10);
            break;
        case 2 :
            _mm256_storeu_pd(C, C00);
            _mm256_storeu_pd(&C[4], C10);
            _mm256_storeu_pd(&C[m], C01);
            _mm256_storeu_pd(&C[m + 4], C11);
            break;
        case 3 :
            _mm256_storeu_pd(C, C00);
            _mm256_storeu_pd(&C[4], C10);
            _mm256_storeu_pd(&C[m], C01);
            _mm256_storeu_pd(&C[m + 4], C11);
            _mm256_storeu_pd(&C[2 * m], C02);
            _mm256_storeu_pd(&C[2 * m + 4], C12);
            break;
        case 4 :
            _mm256_storeu_pd(C, C00);
            _mm256_storeu_pd(&C[4], C10);
            _mm256_storeu_pd(&C[m], C01);
            _mm256_storeu_pd(&C[m + 4], C11);
            _mm256_storeu_pd(&C[2 * m], C02);
            _mm256_storeu_pd(&C[2 * m + 4], C12);
            _mm256_storeu_pd(&C[3 * m], C03);
            _mm256_storeu_pd(&C[3 * m + 4], C13);
            break;
        case 5 :
            _mm256_storeu_pd(C, C00);
            _mm256_storeu_pd(&C[4], C10);
            _mm256_storeu_pd(&C[m], C01);
            _mm256_storeu_pd(&C[m + 4], C11);
            _mm256_storeu_pd(&C[2 * m], C02);
            _mm256_storeu_pd(&C[2 * m + 4], C12);
            _mm256_storeu_pd(&C[3 * m], C03);
            _mm256_storeu_pd(&C[3 * m + 4], C13);
            _mm256_storeu_pd(&C[4 * m], C04);
            _mm256_storeu_pd(&C[4 * m + 4], C14);
            break;
        case 6 :
            _mm256_storeu_pd(C, C00);
            _mm256_storeu_pd(&C[4], C10);
            _mm256_storeu_pd(&C[m], C01);
            _mm256_storeu_pd(&C[m + 4], C11);
            _mm256_storeu_pd(&C[2 * m], C02);
            _mm256_storeu_pd(&C[2 * m + 4], C12);
            _mm256_storeu_pd(&C[3 * m], C03);
            _mm256_storeu_pd(&C[3 * m + 4], C13);
            _mm256_storeu_pd(&C[4 * m], C04);
            _mm256_storeu_pd(&C[4 * m + 4], C14);
            _mm256_storeu_pd(&C[5 * m], C05);
            _mm256_storeu_pd(&C[5 * m + 4], C15);
            break;
        }
    }
}