...
def bmm_wrapper(A, B, shape_A, shape_B,
    ...
    dll_path = os.path.join(current_dir, 'cmake-build-debug', 'libbmm_v8.dll')
...
def calc_offsets(a_dims, b_dims, a_strides, b_strides, fuse_points_a, fuse_points_b):
    ...
    for i in range(3):
        dims_a[i], strides_a[i] = a_dims[fuse_points_a[i]:fuse_points_a[i+1]], np.array(a_strides[fuse_points_a[i]:fuse_points_a[i+1]])
...
def calc_offsets_Cpp(a_dims, b_dims, a_strides, b_strides, fuse_points_a, fuse_points_b):
    """wrapper function that gourps dimension into batch, left and right using fuse_points like this: abcdef -> a|bcd|ef.
    Then, for each of the 3 new dims, offsets are calculated by invoking calc_offsets_dim.
    """
    dims_a, strides_a, offsets_a = [0] * 3, [0] * 3, [0] * 3
    ...
    for i in range(3):
        ...
        offsets_a[i] = calc_offsets_dim_WRAPPER(dims_a[i], strides_a[i], len(dims_a[i]))

    for i in range(3):
        dims_b[i], strides_b[i] = np.array(b_dims[fuse_points_b[i]:fuse_points_b[i+1]]), np.array(b_strides[fuse_points_b[i]:fuse_points_b[i+1]])
        offsets_b[i] = calc_offsets_dim_WRAPPER(dims_b[i], strides_b[i], len(dims_b[i]))

    return offsets_a, offsets_b


def calc_offsets_dim_WRAPPER(dims, strides, n):
    if n == 0:
        return np.zeros(1, dtype=np.int32)
    offsets = np.zeros(prod(dims), dtype=np.int32)
    ...
    lib = ctypes.CDLL(dll_path)
    # set up C++ function
    lib.my_fill.argtypes = [
        ctypes.c_void_p,  # Pointer to offsets
        ctypes.c_void_p,  # Pointer to dims
        ctypes.c_void_p,  # Pointer to strides
        ctypes.c_void_p,  # Pointer to sizes_block
        ctypes.c_int,     # n
    ]
    ...
    lib.my_fill(
        ...
        sizes_block.ctypes.data_as(ctypes.c_void_p),
        ctypes.c_int(n),
    )
    return offsets
...
def fast_einsum(subscripts, a, b=None):
    if b is None:
        ...
        t1 = time.time()
        offsets_a, offsets_b = calc_offsets_Cpp(a_dims, b_dims, a_strides, b_strides, fuse_points_a, fuse_points_b)
        t2 = time.time()
