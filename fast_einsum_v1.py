...
def bmm_wrapper(A, B):
    ...
    dll_path = os.path.join(current_dir, 'cmake-build-debug', 'libbmm_v1.dll')
...
def parse(s, shape_a, shape_b):
    ...
    str_b = str_b_in + "->" + batch_idxs + right_idxs + contract_idxs
    str_res = batch_idxs + left_idxs + right_idxs + "->" +str_c_in

    shape_a = tuple([dim for dim in (batch_dim, left_dim, contract_dim)])
    shape_b = tuple([dim for dim in (batch_dim, right_dim, contract_dim)])
