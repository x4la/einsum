...
def calc_offsets_dim2(dims, strides, n):
    ...
    fill(offsets_ptr, dims, strides, sizes_block, n)
    return offsets
...
def fast_einsum(subscripts, a, b=None):
    if b is None:
        ...
        a1_dims = a1.shape
        a1_strides = tuple(s//a1.itemsize for s in a1.strides)
        dims_batch_a, strides_batch_a = a1_dims[0:fuse_points_a[0]], np.array(a1_strides[0:fuse_points_a[0]])
        dims_left_a, strides_left_a = a1_dims[fuse_points_a[0]:fuse_points_a[1]], np.array(a1_strides[fuse_points_a[0]:fuse_points_a[1]])
        dims_contract_a, strides_contract_a = a1_dims[fuse_points_a[1]:fuse_points_a[2]], np.array(a1_strides[fuse_points_a[1]:fuse_points_a[2]])
        t1 = time.time()
        offsets_batch_a = calc_offsets(dims_batch_a, strides_batch_a)
        offsets_left_a = calc_offsets(dims_left_a, strides_left_a)
        print(dims_batch_a, strides_batch_a)
        print(dims_left_a, strides_left_a)
        print(dims_contract_a, strides_contract_a)
        print("this was what i needed")
        offsets_contract_a = calc_offsets(dims_contract_a, strides_contract_a)
        t2 = time.time()

        b1 = single_einsum(str_b, b)
        b1_dims = b1.shape
        b1_strides = tuple(s//b1.itemsize for s in b1.strides)
        dims_batch_b, strides_batch_b = b1_dims[0:fuse_points_b[0]], np.array(b1_strides[0:fuse_points_b[0]])
        dims_contract_b, strides_contract_b = b1_dims[fuse_points_b[0]:fuse_points_b[1]], np.array(b1_strides[fuse_points_b[0]:fuse_points_b[1]])
        dims_right_b, strides_right_b = b1_dims[fuse_points_b[1]:fuse_points_b[2]], np.array(b1_strides[fuse_points_b[1]:fuse_points_b[2]])
        offsets_batch_b = calc_offsets(dims_batch_b, strides_batch_b)
        offsets_contract_b = calc_offsets(dims_contract_b, strides_contract_b)
        offsets_right_b = calc_offsets(dims_right_b, strides_right_b)

        t3 = time.time()
        c = bmm_wrapper(a, b, desired_shape_a, desired_shape_b, offsets_batch_a, offsets_left_a, offsets_contract_a, offsets_batch_b, offsets_contract_b, offsets_right_b).reshape(shape_res)
        t4 = time.time()
        c = single_einsum(str_res, c)
        return 0, [0, t2-t1, t3-t2, t4-t3, 0]
...
def parse(s, shape_a, shape_b):
    ...
    fuse_points_a = [len(batch_idxs), len(left_idxs)+len(batch_idxs), len(contract_idxs)+len(left_idxs)+len(batch_idxs)]
    fuse_points_b = [len(batch_idxs), len(contract_idxs)+len(batch_idxs), len(right_idxs)+len(contract_idxs)+len(batch_idxs)]
